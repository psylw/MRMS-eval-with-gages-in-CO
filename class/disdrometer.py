...
def get_disdrom(data_folder):
    filenames = listdir(data_folder+'\\disdrometer')
    filenames.sort(reverse=True)
    # first format
    dis1 = pd.read_csv(data_folder +'\\disdrometer\\'+ filenames[0]).drop(
    ...
    # second
    dis2 = [pd.read_csv(data_folder +'\\disdrometer\\'+ filenames[i]) for i in np.arange(1,5,1)]
    dis2 = pd.concat(dis2)
    ...
    #disdrom = disdrom[~disdrom.index.duplicated(keep='first')].resample('1Min').asfreq()
    ...
    rate = (disdrom.accum.rolling(15,min_periods=1).sum())*(60/15)

    disdrom['15_int'] = rate

    disdrom = disdrom.drop(['total_accum'],axis=1)

    coord = [(40.706725,254.360608)] # need actual location

    disdrom = dict(zip(coord, [disdrom]))
